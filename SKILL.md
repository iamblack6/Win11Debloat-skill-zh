...
翻译只改了 `Config/` 下的 JSON 文件，两种运行方式：

**方式一：替换原版 Config**

将 `Win11Debloat-cn/Config/` 下的三个 JSON 覆盖到英文原版 `Win11Debloat/Config/`，然后运行 `Win11Debloat\Win11Debloat.ps1`。

**方式二：直接运行翻译目录**

本 Skill 通过完整复制项目目录来创建翻译副本，因此 `Win11Debloat-cn/` 本身就是可运行的项目，直接执行：

```powershell
.\Win11Debloat-cn\Win11Debloat.ps1
# 或双击 Win11Debloat-cn\Run.bat
```
